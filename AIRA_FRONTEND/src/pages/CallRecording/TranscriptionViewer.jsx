...
const TranscriptionViewer = () => {
  ...
  const handleDownloadRecording = () => {
    if (call?.recordingUrl) {
      ...
      link.download = `recording_${call.phoneNumber}_${formatInIST(call.startTime, 'yyyyMMdd_HHmmss')}.wav`;
  ...
  const handleDownloadTranscript = () => {
    if (transcript) {
      ...
      const link = document.createElement('a');
      link.href = url;
      link.download = `transcript_${call.phoneNumber}_${formatInIST(call.startTime, 'yyyyMMdd_HHmmss')}.json`;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          ...
          <div>
            ...
            <p className="font-medium text-gray-900">
              {formatInIST(call.startTime, 'MMM dd, yyyy HH:mm')}
            </p>
          </div>
          <div>
                  ...
                  >
                    ...
                    {message.timestamp && (
                      <p className="text-xs text-gray-500 mt-1">
                        {formatInIST(message.timestamp, 'HH:mm:ss')}
                      </p>
                    )}
                  </div>
                </div>
