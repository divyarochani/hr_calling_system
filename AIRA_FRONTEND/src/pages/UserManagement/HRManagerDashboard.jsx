  ...
  const fetchData = async () => {
    try {
      const [statsRes, callsRes, candidatesRes] = await Promise.all([
        getDashboardStats(),
        getAllCalls({ limit: 10 }),
        getAllCandidates({ limit: 10 })
      ]);
      
      // New backend returns data directly
      setStats({
        totalCalls: callsRes.total || 0,
        callsToday: 0, // TODO: Calculate from calls
        interviewsScheduled: 0,
      ...
      if (callsRes.calls) {
        callsRes.calls.slice(0, 5).forEach(call => {
          if (call.status === 'completed') {
            activities.push({
              id: call.id,
              action: `Call completed with ${call.phone_number}`,
      ...
      if (candidatesRes.candidates) {
        candidatesRes.candidates.slice(0, 3).forEach(candidate => {
          if (candidate.next_round_availability) {
            activities.push({
              id: candidate.id,
              action: `Interview scheduled for ${candidate.candidate_name || candidate.phone_number}`,
