    ...
    const fetchCandidates = async () => {
        try {
            ...
            const completedCandidates = response.data.filter(
                c => c.callStatus === 'Completed' || c.overallScore !== null
            );
                        ...
                        {candidates.map((candidate) => (
                            <tr key={candidate._id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                                    ...
                                    <div className="text-sm text-gray-900 dark:text-white">
                                        {candidate.overallScore ? (
                                            <span className={`font-semibold ${
                                                candidate.overallScore >= 7 ? 'text-green-600 dark:text-green-400' :
                                                candidate.overallScore >= 5 ? 'text-yellow-600 dark:text-yellow-400' :
                                ...
                                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <button
                                        onClick={() => handleShowConversation(candidate._id)}
