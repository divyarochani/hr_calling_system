...
const InterviewCalendar = () => {
  ...
  const fetchCandidates = async () => {
    try {
      ...
      const scheduledCandidates = response.data.filter(
        c => c.nextRoundAvailability && (c.callStatus === 'Completed' || c.overallScore >= 7)
      );
          ...
          <div className="grid grid-cols-7 gap-2">
            ...
            {Array.from({ length: daysInMonth }).map((_, i) => {
              ...
              const hasInterview = candidates.some(c => {
                if (!c.nextRoundAvailability) return false;
                
                const availability = c.nextRoundAvailability.toLowerCase();
                
                // Try to parse the date
                try {
                  // Check for explicit date match (e.g., "2026-02-20", "20/02/2026", "Feb 20")
                  const dateStr = c.nextRoundAvailability;
                  ...
                  if (dayPattern.test(availability)) {
