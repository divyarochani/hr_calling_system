...
const CallHistoryScreen = () => {
  ...
  const filteredCalls = calls.filter(call => {
    const matchesSearch = call.phoneNumber?.includes(searchTerm) || 
                         call.candidateId?.candidateName?.toLowerCase().includes(searchTerm.toLowerCase());
                ...
                {filteredCalls.map((call) => (
                  <tr key={call._id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-white">
                      {call.candidateId?.candidateName || call.phoneNumber || 'Unknown'}
                    </td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm">
                      <div className="flex items-center gap-2">
                        {call.candidateId && (
                          <button
                            onClick={() => navigate(`/candidate/${call.candidateId._id}`)}
